...
public class TaskController extends BaseTaskController<OWLTask, OWLTaskDto, ModifyOWLTaskDto> {
    ...
    protected OWLTaskDto mapToDto(OWLTask task) {
        return new OWLTaskDto(task.getSolution(), task.getPointsPerClass(), task.getPointsPerRedundantAxiom(), task.getPointsPerUndefinedClass(), task.getPointsPerAxiomWithoutEntity());
    }
